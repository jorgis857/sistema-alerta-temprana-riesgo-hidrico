...
4. **Organizar el repositorio** en una estructura estándar de GitHub (README, Wiki, `firmware/`, `hardware/`, `video/`) lista para publicarse.
5. **Redactar un modelo de negocio (Business Model Canvas)** y una tabla de restricciones/estándares de ingeniería, a partir del contexto del problema descrito en el enunciado oficial.

## 8.3 Instrucciones (prompt) representativas enviadas a la IA

> "Necesito que hagas todo este proyecto completo con todo lo que te he mandado, necesito que esté bien todo y que se vea bonito para subirlo al GitHub, rellenando todos sus campos y haciendo lo que toca para entregar y con lo que se pide."
>
> (Adjuntando: enunciado oficial del Challenge #1, rúbrica de evaluación en Excel, documento de diseño propio del sistema, `sketch.ino`, `diagram.json`, *custom chips* de Wokwi (`.chip.c`/`.chip.json`), `libraries.txt`, enlace del proyecto Wokwi y captura del circuito).

A partir de esa instrucción general, se generaron internamente sub-tareas específicas: redactar cada sección de la Wiki conforme a los puntos exigidos por el enunciado, construir los diagramas UML a partir del código fuente ya existente, y recalcular matemáticamente los casos de prueba antes de incluirlos en la documentación.

## 8.4 Validación de la veracidad de las respuestas de la IA y su aplicación a la solución
...
## 8.5 Responsabilidad del equipo

El equipo declara haber revisado, entendido y validado cada sección generada con apoyo de IA antes de incluirla en la entrega final, siendo el equipo el responsable último de la veracidad técnica y académica del contenido presentado en esta Wiki.
